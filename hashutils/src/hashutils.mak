...
# Target definitions
TARGET_1="$(BINDIR)\crc32sum.$(TARGET_EXTENSION)"
LINK_OBJS_1= \
	"$(OBJDIR)\crc32sum.obj"

TARGET_2="$(BINDIR)\md4sum.$(TARGET_EXTENSION)"
LINK_OBJS_2= \
	"$(OBJDIR)\md4sum.obj"

TARGET_3="$(BINDIR)\md5sum.$(TARGET_EXTENSION)"
LINK_OBJS_3= \
	"$(OBJDIR)\md5sum.obj"

TARGET_4="$(BINDIR)\sha1sum.$(TARGET_EXTENSION)"
LINK_OBJS_4= \
	"$(OBJDIR)\sha1sum.obj"

TARGET_5="$(BINDIR)\ed2ksum.$(TARGET_EXTENSION)"
LINK_OBJS_5= \
	"$(OBJDIR)\ed2ksum.obj"
...
DEFAULT : "$(BINDIR)" "$(OBJDIR)" $(TARGET_1) $(TARGET_2) $(TARGET_3) $(TARGET_4) $(TARGET_5)
!IF $(MAKE_DOPOST) > 0
	 @cd "$(BINDIR)"
	-@md5sum32 *.exe *.dll *.lib 1>"$(BASE_NAME).md5"
	 @cd ..
!ENDIF
...
$(TARGET_2) : $(LINK_OBJS_2)
	$(LINK) @<<
	$(LINKFLAGS) $(LINK_OBJS_2) /OUT:$@
<<

$(TARGET_3) : $(LINK_OBJS_3)
	$(LINK) @<<
	$(LINKFLAGS) $(LINK_OBJS_3) /OUT:$@
<<

$(TARGET_4) : $(LINK_OBJS_4)
	$(LINK) @<<
	$(LINKFLAGS) $(LINK_OBJS_4) /OUT:$@
<<

$(TARGET_5) : $(LINK_OBJS_5)
	$(LINK) @<<
	$(LINKFLAGS) $(LINK_OBJS_5) /OUT:$@
<<
...
"$(OBJDIR)\crc32sum.obj" :
	$(CPP) $(CPPFLAGS) /Fo$@ /D "WHTYPE_CRC32" "$(BASE_NAME).c"

"$(OBJDIR)\md4sum.obj" :
	$(CPP) $(CPPFLAGS) /Fo$@ /D "WHTYPE_MD4" "$(BASE_NAME).c"

"$(OBJDIR)\md5sum.obj" :
	$(CPP) $(CPPFLAGS) /Fo$@ /D "WHTYPE_MD5" "$(BASE_NAME).c"

"$(OBJDIR)\sha1sum.obj" :
	$(CPP) $(CPPFLAGS) /Fo$@ /D "WHTYPE_SHA1" "$(BASE_NAME).c"

"$(OBJDIR)\ed2ksum.obj" :
	$(CPP) $(CPPFLAGS) /Fo$@ /D "WHTYPE_ED2K" "$(BASE_NAME).c"
